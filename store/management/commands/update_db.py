...
class Command(BaseCommand):
    ...
    def write_history(self):
        ...
        for item in search_array:
            try:
                stackable_items = True if len(search_array.filter(item_name__contains=item.item_name).filter(
                    item_name__contains=":")) != 0 else False
        ...
        for item in items_from_library:
            stackable_items = True if len(
                search_array.filter(item_name__contains=item.name).filter(item_name__contains=":")) != 0 else False
            ...
            for v in value:
                if v[3] != 'VendorX' and v[2] != '':
                    ...
                    if _.count() == 0:
                        ...
                        print(f'Updating vendor_name {vendor_update.vendor_name} '
                              f'to {v[2]} in {v[1]}:  X:{v[3]}, Y:{v[4]}, Z:{v[5]}')
                        try:
                    ...
                    try:
                        ...
                        print(f'Mapping does not exist item_name:{v[6]} '
                              f'item_description:{v[7]} item_id:{v[10]}  item_hue:{v[11]}')
                        mapping = None
